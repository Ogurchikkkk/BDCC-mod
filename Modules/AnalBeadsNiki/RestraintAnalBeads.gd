...
func doStruggle(_pc, _minigame:MinigameResult):
	...
	if(failChanceLowScore(_pc, 15, _minigame)):
		text = "{user.name} clenches, attempting to push the beads out. Instead, {user.his} efforts cause it to [b]slide deeper[/b], the plug nestling snugly against {user.his} inner walls."
		damage = -0.5
		lust = scaleDamage(10)
	elif(_handsFree && _armsFree):
		text = "Because {user.name}'s hands are free {user.he} {user.verbS('manage')} to just remove the beads"
		damage = 1.0
		lust = 10
	elif(_legsFree):
		text = "{user.name} squirms and wiggles {user.his} rear, trying to push the beads out of {user.his} butt"
		damage = calcDamage(_pc, _minigame)
		stamina = 5
		lust = scaleDamage(5)
	else:
		text = "{user.name} desperatelly squirms, trying to make the anal beads fall out. Not being able to spread {user.his} legs makes it very hard"
	...
	if(damage < 1.0):
		if(_pc.isPlayer() && failChance(_pc, 40) && _pc.getInventory().hasSlotEquipped(InventorySlot.UnderwearBottom)):
			if(_pc.getInventory().getEquippedItem(InventorySlot.UnderwearBottom).coversBodypart(BodypartSlot.Anus)):
				text += " The beads are pressing into your panties."
				damage /= 2.0
...
func processStruggleTurn(_pc, _isActivelyStruggling):
	if(failChance(_pc, 10) || _isActivelyStruggling):
		return {"text": "The beads shift inside {user.name} while {user.he} {user.verbS('squirm')}", "lust": scaleDamage(5)}
